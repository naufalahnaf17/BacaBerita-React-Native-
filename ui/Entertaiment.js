...
function ArtikelEntertaiment({title , urlImage , navigation , url}){
    return(
...
class EntertaimentContent extends Component{
    ...
    render(){
        ...
        return(
            <FlatList 
                data={artikel}
                renderItem={({ item }) => <ArtikelEntertaiment title={item.title} urlImage={item.urlToImage} url={item.url} navigation={this.props.navigation} /> }
...
export default class Entertaiment extends Component{
    ...
    _getTopArticles(){
        ...
        axios({
                method : 'get',
                url : 'http://newsapi.org/v2/top-headlines?country='+country+'&category=entertainment&apiKey='+apiKey,
            }).then((response) => {
    ...
    render(){
        ...
        if(isLoading){
            return(
                <Fragment>
                    <EntertaimentContent topArticles={topArticles} navigation={this.props.navigation} />
                </Fragment>
            );
        }else{
            return(
