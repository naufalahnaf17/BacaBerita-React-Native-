...
function ArtikelScience({title , urlImage , navigation , url}){
    return(
...
class ScienceContent extends Component{
    ...
    render(){
        ...
        return(
            <FlatList 
                data={artikel}
                renderItem={({ item }) => <ArtikelScience title={item.title} urlImage={item.urlToImage} url={item.url} navigation={this.props.navigation} /> }
...
export default class Science extends Component{
    ...
    _getTopArticles(){
        ...
        axios({
                method : 'get',
                url : 'http://newsapi.org/v2/top-headlines?country='+country+'&category=science&apiKey='+apiKey,
            }).then((response) => {
    ...
    render(){
        ...
        if(isLoading){
            return(
                <Fragment>
                    <ScienceContent topArticles={topArticles} navigation={this.props.navigation} />
                </Fragment>
            );
        }else{
            return(
