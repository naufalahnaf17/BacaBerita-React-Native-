...
function ArtikelHealth({title , urlImage , navigation , url}){
    return(
...
class HealthContent extends Component{
    ...
    render(){
        ...
        return(
            <FlatList 
                data={artikel}
                renderItem={({ item }) => <ArtikelHealth title={item.title} urlImage={item.urlToImage} url={item.url} navigation={this.props.navigation} /> }
...
export default class Health extends Component{
    ...
    _getTopArticles(){
        ...
        axios({
                method : 'get',
                url : 'http://newsapi.org/v2/top-headlines?country='+country+'&category=health&apiKey='+apiKey,
            }).then((response) => {
    ...
    render(){
        ...
        if(isLoading){
            return(
                <Fragment>
                    <HealthContent topArticles={topArticles} navigation={this.props.navigation} />
                </Fragment>
            );
        }else{
            return(
